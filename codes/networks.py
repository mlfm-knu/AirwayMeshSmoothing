...
class GNNVer(nn.Module):
    def __init__(self, device):
        ...
        #13GCN layers==> we have 13 message passing steps and then a linear output layer   
        ...
        #apply linear transformation to the imcoming data
    ...
    def forward(self, data):
        ...
        #first message passing layer (transformation) 6: is the node embedding size
...
class GNNVer1(nn.Module):
    def __init__(self, device):
        super(GNNVer1, self).__init__()
        self.device = device
        
        feu = [6, 32, 64, 128, 256, 256, 512, 512, 256, 256, 128, 64, 32, 16, 3]
        self.conv1  = GCNConv(feu[0], feu[1]) 
        self.conv2  = GCNConv(feu[1], feu[2])
        self.conv3  = GCNConv(feu[2], feu[3])
        self.conv4  = GCNConv(feu[3], feu[4])
        ...
        self.linear1 = nn.Linear(feu[12], feu[13]) 
...
class GNNFac1(nn.Module):
    def __init__(self, device):
        super(GNNFac1, self).__init__()
        self.device = device
